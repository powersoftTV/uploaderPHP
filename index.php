...
<tr>
    ...
    <? if(isset($_GET['order'])){
    	if($ord=="size"){
			?>
    <input type="radio" name="order" value="size" checked="checked"/>size 
    ...
    <input type="radio" name="order" value="date" />date 
    <input type="radio" name="order" value="name" checked="checked"/>name
    	<? }
		if($ord=="date"){
			?>
    <input type="radio" name="order" value="size" />size 
    <input type="radio" name="order" value="date" checked="checked"/>date 
    <input type="radio" name="order" value="name" />name
    	<? }
    }
	else {
	...
	if(isset($_GET['desc'])) { 
		if($des=="descending"){
			?>
    		<input type="checkbox" name="desc" value="descending" checked="checked"/>descending <? ;} 
		else {
...
<li >
       ...
       <? if(file_exists("uploads/tumb/".$file)){
					...
					$srcy= $size[1];
					?>
            		<img title="<?=$size[0]?> x <?=$size[1]?>" src="uploads/tumb/<?=$file?>" alt="<?=$file?>" /> <? ;} ?>
        ...
        <? if(in_array($ext, $valid_p_types)) { ?>
				...
				<? $upfile="uploads/".$file;
				$link =$_SERVER['PHP_SELF']."?del=".$file."&amp;p=".$page."&amp;order=".$ord;
				if($_GET[desc] && $_GET[desc]!="") $link=$link."&amp;desc=".$des;
				?><a href="<?=$link?>"><font color="#cc0000">remove</font></a> <?
...
<?	
$link =$_SERVER['PHP_SELF']."?id=".$id."&amp;order=".$ord;
if($_GET[desc] && $_GET[desc]!="") $link=$link."&amp;desc=".$des;
mypagesnew($num,$page,$pages, $link."&amp;");
?>
</div>
